...
The runs-on string selects the runner host. My personal device is being used to run the test so "self-hosted" is assigned to runs-on. 

2. In `main.yml`, on which branch do the jest tests run when a push to main branch is made?
The jest tests runs on the main branch. The tests are triggered when code is pushed to the main branch.


3. In `main.yml`, on which branch do the jest tests run when a pull request is submitted to the deploy branch?
The jest tests run on the deploy branch. When a pull request is submitted, it triggered on pull_request. 
